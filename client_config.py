...
bass_path = Path(__file__).parent


def resolve_relative_path(*path):
    return str(Path(bass_path, *path))
...
class client_config:
    ...
    channel_ca = resolve_relative_path("bin/ca.crt")  # 采用channel协议时，需要设置链证书,如采用rpc协议通信，这里可以留空
    channel_node_cert = resolve_relative_path("bin/sdk.crt")  # 采用channel协议时，需要设置sdk证书,如采用rpc协议通信，这里可以留空
    channel_node_key = resolve_relative_path("bin/sdk.key")  # 采用channel协议时，需要设置sdk私钥,如采用rpc协议通信，这里可以留空
    channel_en_crt = resolve_relative_path("bin/gmensdk.crt")  # 仅国密双证书使用，加密证书
    channel_en_key = resolve_relative_path("bin/gmensdk.key")  # 仅国密双证书使用，加密key
    ...
    contract_info_file = resolve_relative_path("bin/contract.ini")  # 保存已部署合约信息的文件
    account_keyfile_path = resolve_relative_path("bin/accounts")  # 保存keystore文件的路径，在此路径下,keystore文件以 [name].keystore命名
    ...
    solc_path = resolve_relative_path("./bin/solc/v0.4.25/solc")
    gm_solc_path = resolve_relative_path("./bin/solc/v0.4.25/solc-gm")
    solcjs_path = resolve_relative_path("./solcjs")

    logdir = resolve_relative_path("bin/logs")  # 默认日志输出目录，该目录必须先建立
